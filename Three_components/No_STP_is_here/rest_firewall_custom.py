...
class FirewallController(ControllerBase):
    ...
    def regist_ofs(dp):
        ...
        if((dp.id <0x41) or (dp.id >=0x60)):           #Ahmed notes
            return
        try:
            f_ofs = Firewall(dp)
    ...
    def packet_in_handler(msg):
        ...
        if((msg.datapath.id <0x41) or (msg.datapath.id >=0x60)):        #Ahmed notes:  define any switch has id lower than 20 don't execute it as firewall
            return
