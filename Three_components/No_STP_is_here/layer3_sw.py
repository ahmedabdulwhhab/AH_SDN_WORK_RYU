...
# DELETE /l3sw/{switch_id}
#
# * delete data of specific vlan group
# DELETE /l3sw/{switch_id}/{vlan_id}
#
...
class RestRouterAPI_L3(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        super(RestRouterAPI_L3, self).__init__(*args, **kwargs)
    ...
    def datapath_handler(self, ev):
        if((ev.dp.id <0x11) or (ev.dp.id >=0x1f)):           #Ahmed notes
            return
        if ev.enter:
    ...
    def _stats_reply_handler(self, ev):
        msg = ev.msg
        dp = msg.datapath
        if((dp.id <0x11) or (dp.id >=0x1f)):           #Ahmed notes
            return
...
class RouterController(ControllerBase):
    ...
    def register_router(cls, dp):
        ...
        cls._LOGGER.info('Join as Layer 3 swich.', extra=dpid)
    ...
    def unregister_router(cls, dp):
        ...
        if dp.id in cls._ROUTER_LIST:
            ...
            dpid = {'sw_id': dpid_lib.dpid_to_str(dp.id)}
            cls._LOGGER.info('Leave Layer 3 switch .', extra=dpid)
