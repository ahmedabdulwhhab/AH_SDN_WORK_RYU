...
class RouterController(ControllerBase):
    ...
    def register_router(cls, dp):
        ...
        if(dp.id==1):
                                ...
                                match = parser.OFPMatch(eth_type=ether_types.ETH_TYPE_IP,icmpv4_type=8, ipv4_src="172.16.10.1/24", ip_proto=inet.IPPROTO_ICMP)                            
    ...
    def packet_in_handler(self, msg, header_list):
        ...
        if ofproto.OFP_VERSION == ofproto_v1_2.OFP_VERSION or \
            ...
            if msg.reason == ofproto.OFPR_INVALID_TTL:
                ...
                return
        """ In the name of Allah, Let us starting"""
        ############################################
        if(ofproto.OFP_VERSION == ofproto_v1_3.OFP_VERSION):
            datapath = msg.datapath
            ofproto = datapath.ofproto
            #confirm ofproto_v1_3       
            ...
            if eth.ethertype == ether_types.ETH_TYPE_LLDP:
                ...
                if eth.ethertype == ether_types.ETH_TYPE_IP:
                        ...
                        if pkt_arp:
                            ...
                            print("Ahmed, at dpid ",dpid," dpipkt_arp, pkt_arp.opcode = ", pkt_arp.opcode, " srcip =   ",src_ip, "dstip = "  ,dstip)
                    
                        # if ICMP Protocol
                        elif pkt_icmp:
                            print("pkt_icmp.type = ",pkt_icmp.type)
                            srcip = pkt_ip.src
                            dstip = pkt_ip.dst
                            
                            print("Ahmed, at dpid ",dpid," pkt_icmp,  srcip =   ",srcip, "dstip = "  ,dstip) 
                            if(dpid==1) and (srcip == "172.16.10.10") and ( dstip =="172.16.20.10"):
                                match = parser.OFPMatch(eth_type=ether_types.ETH_TYPE_IP,icmpv4_type=8, ipv4_src=srcip, ipv4_dst=dstip, ip_proto=inet.IPPROTO_ICMP)                            
                        ...
                        #  if TCP Protocol
                            ...
                            print("Ahmed, at dpid ",dpid," pkt_tcp,  srcip =   ",src_ip, "dstip = "  ,dstip," src_port ", t.src_port," dst_port ",t.dst_port)
                        ...
                        #  If UDP Protocol 
                            ...
                            print("Ahmed , at dpid ",dpid," pkt_udp,  srcip =   ",src_ip, "dstip = "  ,dstip," src_port ", u.src_port," dst_port ",u.dst_port)
#                        else:
#                            print ("pkt is ",pkt)
