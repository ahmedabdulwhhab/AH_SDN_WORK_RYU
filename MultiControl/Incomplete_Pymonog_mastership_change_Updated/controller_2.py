...
##############new code
    ...
    def _monitor(self):
        while True:
            the_other_contrroler_state = self.Get_Other_controller_status()
            print("inside _monitor , self.current_role is ",self.current_role," while the_other_contrroler_state is ", the_other_contrroler_state)
            if(the_other_contrroler_state=="ofp.OFPCR_ROLE_MASTER"):
                self.current_role="ofp.OFPCR_ROLE_SLAVE"
            ...
            hub.sleep(10)        #check Master status every 5 seconds
    ...
    def update_pymongo_db(self,current_role):
        ...
        for x in self.mycol.find():
                ...
                if(x['contr']==str(self.ctrl_id)):
                        ...
                        print("Data base update to be ", curr_role)
        
        
    def Get_Other_controller_status(self):
        self.mycol = self.mydb["customers"]
        curr_role=self. current_role
        
        # Updating fan quantity form 10 to 25.
        myfilter = { "contr": str(self.ctrl_id-1) }
        ...
        for x in self.mycol.find():
                #print(x)
                if(x['contr']==str(self.ctrl_id-1)):
                    print("Other_controller state is ",x['role'])
                return x['role']
        return "NULL"        
