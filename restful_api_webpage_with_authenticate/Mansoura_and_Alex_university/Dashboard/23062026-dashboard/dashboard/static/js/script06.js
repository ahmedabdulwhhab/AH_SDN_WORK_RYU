...
confirmButton.addEventListener("click", () => {
  ...
  if (selectedOption === "get-status") {
	...
	if((switch_id > 9) & (switch_id < 100))
	{
		api_url = "http://192.168.127.133:8085/firewall/module/enable/00000000000000" + switch_id;
	}
  ...
  headers.append('Access-Control-Allow-Credentials', 'true');
    fetch(
      ...
      .then((data) => showAlert(data));
		...
		if((switch_id > 9) & (switch_id < 100))
	{
		api_url = "http://192.168.127.133:8085/firewall/rules/00000000000000"+switch_id;
	}
    ...
    const protocol = document.getElementById("protocol").value;
    //const pirority = document.getElementById("pirority").value;
	var data=JSON.stringify({ 'nw_src': sourceIp, 'nw_dst': destinationIp,'nw_proto': protocol,'actions': 'DENY' });
	alert(data)
    api_url = "http://192.168.127.133:8085/firewall/rules/000000000000000"+switch_id;
	if((switch_id > 9) & (switch_id < 100))
	{
